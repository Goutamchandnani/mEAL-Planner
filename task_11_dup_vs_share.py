"""
Task 11: Diversity analysis - Duplicate Avoidance.
This file consolidates all necessary code for implementing Duplicate Avoidance and comparing it with the Fitness Sharing algorithm.
"""
...
def euclidean_distance_task_6(ind1: List[Tuple[Food, float]], ind2: List[Tuple[Food, float]]) -> float:
...
def sharing_function_task_6(distance: float, sigma_share: float, alpha: float) -> float:
    if distance < sigma_share:
        return 1.0 - (distance / sigma_share) ** alpha
    else:
        return 0.0

def apply_fitness_sharing_task_6(population: List[List[Tuple[Food, float]]], sigma_share: float, alpha: float):
    n = len(population)
    ...
    for i in range(n):
        niche_count = 0.0
        for j in range(n):
            distance = euclidean_distance_task_6(population[i], population[j])
            niche_count += sharing_function_task_6(distance, sigma_share, alpha)
...
def evolve_meal_div_task_6(
    population_size: int = 100,
    generations: int = 50,
    min_fitness_threshold: float = 5.0,
    cxpb: float = 0.7,
    mutpb: float = 0.2,
    sigma_share: float = SHARING_RADIUS,
    alpha: float = ALPHA
) -> Meal:
    creator.create("FitnessMax", base.Fitness, weights=(1.0,))
    creator.create("Individual", list, fitness=creator.FitnessMax)
    ...
    for gen in range(generations):
        ...
        apply_fitness_sharing_task_6(population, sigma_share, alpha)
...
# --- Content from meal_evolution_duplicates_task_11.py ---
def evolve_meal_duplicates_task_11(
    population_size: int = 100,
    generations: int = 50,
    min_fitness_threshold: float = 5.0,
    cxpb: float = 0.7,
    ...
    for gen in range(generations):
        ...
        hof.update(population)
        if hof[0].fitness.values[0] >= min_fitness_threshold:
            print(f"Termination condition met: Max fitness of {hof[0].fitness.values[0]} reached.")
            break
...
if __name__ == "__main__":
    ...
    print("\n--- Running Genetic Algorithm with Fitness Sharing (mEAl_Div) for comparison (Task 11) ---")
    sharing_ga_fitness = []
    for i in range(10): # Reduced trials for quicker execution in a single file
        print(f"  Trial {i+1}/10...", end=" ", flush=True)
        best_meal = evolve_meal_div_task_6(
            population_size=250,
            generations=200,
            mutpb=0.2,
            cxpb=0.6,
            sigma_share=SHARING_RADIUS,
            alpha=ALPHA
        )
        sharing_ga_fitness.append(best_meal.calculate_fitness())
        print(f"Fitness: {sharing_ga_fitness[-1]:.4f}")

    print("\n--- Running Genetic Algorithm with Duplicate Avoidance for comparison (Task 11) ---")
    duplicate_ga_fitness = []
    for i in range(10): # Reduced trials for quicker execution in a single file
        print(f"  Trial {i+1}/10...", end=" ", flush=True)
        best_meal = evolve_meal_duplicates_task_11(
            population_size=250,
            generations=200,
            mutpb=0.2,
            cxpb=0.6
        )
        duplicate_ga_fitness.append(best_meal.calculate_fitness())
        print(f"Fitness: {duplicate_ga_fitness[-1]:.4f}")

    print("\n--- Statistical Comparison (Welch's t-test) ---")
    print(f"Fitness Sharing GA Average Fitness: {np.mean(sharing_ga_fitness):.4f} ± {np.std(sharing_ga_fitness):.4f}")
    print(f"Duplicate Avoidance GA Average Fitness: {np.mean(duplicate_ga_fitness):.4f} ± {np.std(duplicate_ga_fitness):.4f}")

    t_statistic_fit, p_value_fit = stats.ttest_ind(sharing_ga_fitness, duplicate_ga_fitness, equal_var=False)
    ...
    if p_value_fit < alpha:
        ...
        if np.mean(duplicate_ga_fitness) > np.mean(sharing_ga_fitness):
            print("  Duplicate Avoidance GA performs significantly better in terms of fitness.")
        else:
